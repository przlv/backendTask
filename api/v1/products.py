...
async def create_products_for_shift_tasks(
    new_products: List[ProductData], db: AsyncSession = Depends(get_db)
):
    try:
...
async def aggregation_products(
    id_product: int, unique_product_code: str, db: AsyncSession = Depends(get_db)
):
    try:
