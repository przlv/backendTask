...
from crud.shift_assignment import (create_shift_task, get_shift_task,
                                   change_shift_task, get_filtered_shift_tasks)
...
async def get_shift_tasks_filtered(
    ...
    try:
        filtered_tasks = await get_filtered_shift_tasks(
            db, closing_status, batch_number, batch_date,
            shift_start_datetime, shift_end_datetime, offset, limit)
...
async def create_shift_assignment(shift_tasks: List[ShiftTask],
                                  db: AsyncSession = Depends(get_db)):
    try:
...
async def change_shift_assignment(key_task: int,
                                  new_data_task: ShiftTaskChange,
                                  db: AsyncSession = Depends(get_db)):
    try:
