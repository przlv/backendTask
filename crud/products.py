...
async def get_products_on_shift(key_shift: int, db: AsyncSession) -> List[Dict]:
    try:
        result = await db.execute(
            select(ProductDataModel).filter_by(batch_number=key_shift)
        )
        ...
        try:
            for product in new_products:
                ...
                existing_product = await db.execute(
                    select(ProductDataModel).filter_by(
...
async def aggregation_product(
    id_product: int, unique_product_code: str, db: AsyncSession
) -> Dict:
    async with db.begin():
        try:
